...
var pmsgOrigin = nvid('postMessage');
pmsgOrigin = pmsgOrigin.replace(/\/$/, '')


//$('#use_vid').trigger('click');
...
/**
 * [goto_dialog jquery mobile dialog 弹窗跳转]
 * @param  {[type]} where [page id]
 */
function goto_dialog(where) {
	$(":mobile-pagecontainer").pagecontainer("change", "#dialog-" + where, {
		role: "dialog"
	});
}
...
function messageTextToJQ(body) {
	...
	var match, $link, img_url, $body_parts = $(),
		rx = /(https?:\/\/[^ \n\r]+|\n\r|\n|\r)/;
	...
	onMessage: function (verto, dialog, msg, data) {
		...
		*/
			...
			case $.verto.enum.message.display:
				...
				display("对话: <b>" + (dialog.cidString()) + '</b>');
				break;
			default:
				break;
		}
	...
	onDialogState: function (d) {
		...
		switch (d.state) {
			case $.verto.enum.state.ringing:
				...
				$("#dialog-incoming-call-txt").text("来电 从: " + d.cidString());
			...
			case $.verto.enum.state.hangup:
				$("#main_info").html("通话结束原因： " + d.cause);

				///////// postMessage 消息通知

				parentW.postMessage('hangup~' + d.cause, pmsgOrigin);
				$("#ext").val("");
				$("#ext").show();
...
$("#localmutebtn").click(function () {
	...
	display("对话: <b>" + (cur_call.cidString()) + '</b>' + (muted ? " [LOCALLY MUTED]" : ''));

});

$("#localvidmutebtn").click(function () {
	var muted = cur_call.setVideoMute("toggle");
	display("对话:<b> " + (cur_call.cidString()) + '</b>' + (muted ? " [VIDEO LOCALLY MUTED]" : ''));
...
/**
 * [docall 拨打电话]
 * @return {[type]} [vertoHandle]
 */
function docall(caller, callee) {
	// var extNum = $('#ext')
	// if ( !nvid('ext') ) return alert('请输入呼叫号码')

	// if (nvid('ext').length <= 13 ) {
	// 	extNum.val(extNum.data('pre')+nvid('ext'))
	// }
	var extNum = $('#ext')
	var code = $('#ext').data('code')
	var orderType = $('#ext').data('order')
	window.localStorage.setItem('caller', '')
	if (!nvid('ext')) return alert('请输入呼叫号码')
	// if (nvid('ext').length <= 13 ) {
	if (orderType == 4) {
		extNum.val(code + '' + caller + '' + callee)
    // extNum.val(code + '028' + callee)
   ...
   extNum.val(code + '' + caller + '' + callee)
	...
	// }
	extNum.hide();
	extNum.trigger('change');

	if (cur_call) return;

	///////// postMessage 消息通知

	parentW.postMessage('trying~' + phoneHide(nvid('ext')), pmsgOrigin);

	///////// postMessage 消息通知

	$("#main_info").html("正在尝试连接");

	check_vid_res();
	//console.log(outgoingBandwidth, incomingBandwidth);

	cur_call = vertoHandle.newCall({
		destination_number: nvid('ext'),
		caller_id_name: nvid('cidname'),
		caller_id_number: nvid('cid'),
...
// 获取真实号码：阿里隐私号码
function getActualNumber() {
	var extNum = $('#ext')
	var orderType = $('#ext').data('order')
	var sipNumber = $('#ext').data('sip').split(',')
	var phoneNumber = nvid('ext')
	var ran = parseInt(Math.random() * 3, 10)
	if (!nvid('ext')) return alert('请输入呼叫号码')
	if (orderType == 4) {
		parentW.postMessage('aliPrivacy~'  + sipNumber[ran] + '~' + phoneNumber, pmsgOrigin);
	} else {
		parentW.postMessage('aliPrivacy~' + extNum.data('pre') + '~' + phoneNumber, pmsgOrigin);
	}
}
...
function refresh_devices() {
	...
	$("#usecamera").append(new Option("禁用摄像头", "none"));
	$("#usemic").append(new Option("不指定", "any"));
	$("#usespeak").append(new Option("不指定", "any"));
	...
	var o = new Option("屏幕", "screen");
	o.selected = true;

	$("#useshare").append(o);

	$("#usemic").append(new Option("禁用麦克风", "none"));
...
function init() {
	...
	// if (!autocall) {
	// 	pop("#ext", "verto_demo_ext", "80000");
	// }

	pop("#avatar", "verto_demo_avatar", "", function (jq) {
		$("#avatar_img").attr("src", jq.val());
	});
	pop("#cidname", "verto_demo_name", "mjoys_" + generateSalt(6));
	pop("#cid", "verto_demo_cid", nvid('login'));
	...
	pop("#login", "verto_demo_login", nvid('login'));
	pop("#passwd", "verto_demo_passwd", nvid('passwd'));

	pop("#hostName", "verto_demo_hostname", nvid('hostName'));
	//pop("#wsURL", "verto_demo_wsurl", "wss://" + window.location.hostname + ":8082");
	pop("#wsURL", "verto_demo_wsurl", nvid('wsURL'));
	...
	$('#login').data('verto_demo_vid_checked', tmp)
	...
	$("#logoutbtn").click(function () {
		...
		parentW.postMessage('logout', pmsgOrigin);
	...
	$("#loginbtn").click(function () {
		...
		parentW.postMessage('login', pmsgOrigin);
...
$(window).load(function () {
	...
	parentW.postMessage('ready', pmsgOrigin);
...
/**
 * [handleMessage 接收到 pmsg 消息处理]
 * @param  {[type]} event [e]
 * @return {[type]}       [boolean]
 */
function handleMessage(event) {
	event = event || window.event;
	var preNum = $('#ext').data('pre')
	//console.log(nvid('postMessage'), event.origin, event.data);
	...
	if (action == 'call' || action == 'recall') {
		...
		if (!str) return alert('没有传入 电话号');
		$('#ext').val(str);
		$("#callbtn").trigger('click');

	} else if (action == 'hangup') {
		$("#hupbtn2").trigger('click');

	} else if (action === 'aliPrivacy') {
		var caller = str
		var callee = data.split('~')[2]
		docall(caller, callee)
	}
	else {
		console.error('need extend', data)
	}
}
