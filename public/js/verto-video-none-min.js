// js2image -s ./public/assets/custom/fs/js/verto-min.js -i ./public/assets/custom/fs/images/verto_black_web.png 
// mv -i ./public/assets/custom/fs/js/verto-min.xmas.js ./public/assets/build/fs/
(function($) {
	...
	FSRTCattachMediaStream = function(element, stream) {
		if (!element) return console.warn('Error element not exist');
	...
	$.JsonRpcClient.prototype.closeSocket = function() {
		...
		if (self.socketReady()) {
			self._ws_socket.onclose = function(w) {
				$.cookie('verto-callFlag','')
				// alert('fs连接已断开，请刷新重试，以保证电话功能正常使用')
		...
		if (!self.socketReady()) {
			...
			if (self._ws_socket) {
				...
				self._ws_socket.onclose = function(w) {
					...
					console.warn("Websocket Lost " + self.ws_cnt + " sleep: " + self.ws_sleep + "msec");
				...
				self._ws_socket.onopen = function() {
					...
					// 定时发送一个Ping包，解决60S websocket被关闭问题
					self._interval = setInterval(function() {
						self._ws_socket.send(JSON.stringify({ jsonrpc:"2.0", id: null, method: 'ping'}))
					}, 30000)
					var req;
		...
		try {
			...
			if (typeof response === 'object' && 'jsonrpc' in response && response.jsonrpc === '2.0') {
				if ('result' in response && this._ws_callbacks[response.id]) {
					...
					console.log(!self.authing , response.error.code == -32000 , self.options.login , self.options.passwd)
...
(function($) {
	...
	$.verto = function(options, callbacks) {
		...
		if (verto.options.sessid) {
			...
			verto.sessid = generateGUID(); //localStorage.getItem("verto_session_uuid") ||
	...
	$.verto.confMan = function(verto, params) {
		...
		confMan.lt = new $.verto.liveTable(verto, confMan.params.laData.laChannel, confMan.params.laData.laName, $(confMan.params.tableID), {
			...
			"onChange": function(obj, args) {
				$(confMan.params.statusID).text("会议成员: " + " (" + obj.arrayLen() + " 位)");
	...
	$.verto.dialog = function(direction, verto, params) {
		...
		if (dialog.direction == $.verto.enum.direction.inbound) {
			...
			dialog.params.remote_caller_id_name = "呼出电话";
	...
	$.verto.dialog.prototype.setState = function(state) {
		...
		// parentW.postMessage('calluuid~'+dialog.callID, pmsgOrigin);
		// console.log('@@@@@@@@@@@ ', dialog.callID)
	...
	$.verto.dialog.prototype.hangup = function(params) {
		...
		if (params) {
			...
			if (params.cause) {
				...
				if(params.cause ==='BEARERCAPABILITY_NOTAVAIL'){
					alert('今日呼叫次数已达上线，请明日再呼！')
				}
	...
	$.verto.dialog.prototype.cidString = function(enc) {
		var dialog = this;
		var party = (enc ? " &lt;" : " <") +  //dialog.params.remote_caller_id_name + 
	...
	$.verto.dialog.prototype.handleMedia = function(params) {
		...
		var parentW = window;
		var pmsgOrigin = location.origin //$.trim($('#postMessage').val())
		parentW.postMessage('calluuid~'+dialog.callID, pmsgOrigin);
